...
procedure BinToC(const FileName, outfile: string);
var N         : integer; 
    ByteRead  : byte; 
    BufStr    : string;
...
begin
  ...
  try 
    ...
    begin 
      ...
      try
        BufStr := '/* '+FileName+' */'+#13#10+
                  'extern const unsigned char ' + 'BinaryData[' + IntToStr(FluxIn.Size) + '] = {'+#13#10#09#09;
...
begin
  i := ParamCount;
  if i <= 0 then begin
    Writeln('bin2c: Convert binary file to c++ source');
    Writeln('');
    Writeln('\tUsage:  bin2c inputfile [outfile]');
    Exit;
  end;

  if i > 1 then outfile := ParamStr(2)
  else outfile := ParamStr(1) + '.cpp';

  BinToC(ParamStr(1), outfile);
end.
